...
if aws apprunner describe-service --service-arn $SERVICE_ARN > /dev/null 2>&1; then
    ...
    mkdir /app
    chmod 666 /app
    aws apprunner update-service --service-arn $SERVICE_ARN --cli-input-json file:///app/update-service.json
else
    echo "Service does not exist, creating..."
    aws apprunner create-service --cli-input-json file:///app/create-service.json
fi
